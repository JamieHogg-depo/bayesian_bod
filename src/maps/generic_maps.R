...
df_temp <- lga$pop %>% 
  ...
  summarise(N = sum(N),
            .groups = "drop") %>% 
  group_by(Year) %>% 
  mutate(N_c = 2*cut_number(N, 50, labels = FALSE)) %>% 
  ungroup() %>% 
  filter(Year %in% c(2015:2019))

# create map data list
map_temp <- left_join(df_temp,map, by = c("LGA_Code" = "LGA_CODE16")) %>% 
  st_as_sf() %>%
  st_transform(4326)

## Create map - 2017 only ## ---------------------------------------------------
...
temp <- base +
  labs(fill = "Population (percentiles)")+
...
out = arrangeGrob(grobs = list(base_boxes, perth_inset, base_legend), 
                  layout_matrix  = lay,
                  top = textGrob("2017",gp=gpar(fontsize=10)))
jsave(plot = out, filename = "map_population2017.png", base_folder = "plts", square = F)
...
jsave(plot = full_inset_plt, filename = "map_population.png", base_folder = "plts", square = F)
